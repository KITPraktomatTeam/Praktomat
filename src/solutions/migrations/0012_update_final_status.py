...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
		# persist which solutions were the final solutions
		for task in orm['tasks.Task'].objects.all():
			solutions = task.solution_set.filter(author__groups__name = "User")
			solution_ids = map(lambda x:x['id__max'], solutions.values('author').annotate(Max('id')))
			orm.Solution.objects.filter(id__in=solution_ids).update(final=True)
